...
Also seeds 5 pre-built long-term memories so the demo starts in "shift 2" with
institutional fault knowledge already in place (requires AGENT_MEMORY_* vars).

Usage:
    uv run python seed_crestforge.py            # upsert — additive, safe to rerun
    uv run python seed_crestforge.py --flush    # FLUSHDB first (wipes Agent Memory too)
    uv run python seed_crestforge.py --verify   # count + print samples, no writes
    uv run python seed_crestforge.py --no-memory  # skip Agent Memory seeding
"""
...
MACHINES = [
    {"id": "M104", "name": "Alpha Mill",   "type": "cnc_mill",       "location": "zone_a",
     "status": "fault",       "vibration_level": 9.2,  "temperature_c": 68.0, "runtime_hours": 512},
    {"id": "M312", "name": "Delta Comp",   "type": "compressor",     "location": "zone_c",
     "status": "running",     "vibration_level": 1.1,  "temperature_c": 87.0, "runtime_hours": 330},
    {"id": "M207", "name": "Beta Press",   "type": "hydraulic_press","location": "zone_b",
     "status": "maintenance", "vibration_level": 0.3,  "temperature_c": 42.0, "runtime_hours": 891},
    {"id": "M118", "name": "Gamma Belt",   "type": "conveyor",       "location": "zone_a",
     "status": "running",     "vibration_level": 0.8,  "temperature_c": 38.0, "runtime_hours": 487},
    {"id": "M225", "name": "Epsilon Arm",  "type": "robot_arm",      "location": "zone_b",
     "status": "idle",        "vibration_level": 0.0,  "temperature_c": 29.0, "runtime_hours": 204},
    {"id": "M401", "name": "Zeta Mill",    "type": "cnc_mill",       "location": "zone_d",
     "status": "running",     "vibration_level": 2.1,  "temperature_c": 55.0, "runtime_hours": 103},
    {"id": "M309", "name": "Eta Press",    "type": "hydraulic_press","location": "zone_c",
     "status": "running",     "vibration_level": 0.6,  "temperature_c": 44.0, "runtime_hours": 67},
    {"id": "M502", "name": "Theta Conv",   "type": "conveyor",       "location": "zone_d",
     "status": "idle",        "vibration_level": 0.0,  "temperature_c": 31.0, "runtime_hours": 12},
]

ALERTS = [
    {"id": "A301", "machine_id": "M104", "type": "vibration",   "severity": "critical",
     "status": "open",       "triggered_value": 9.2,  "threshold": 8.0},
    {"id": "A308", "machine_id": "M312", "type": "temperature", "severity": "warning",
     "status": "open",       "triggered_value": 87.0, "threshold": 85.0},
    {"id": "A295", "machine_id": "M207", "type": "lubrication", "severity": "warning",
     "status": "resolved",   "triggered_value": 0.4,  "threshold": 0.8},
]

WORK_ORDERS = [
    {"id": "WO1041", "machine_id": "M104", "technician_id": "T03",
     "type": "repair",      "status": "scheduled",   "priority": "urgent",
     "description": "Critical vibration alert A301 — suspected front bearing failure. Inspect and replace SKF 6205 bearing."},
    {"id": "WO1042", "machine_id": "M207", "technician_id": "T07",
     "type": "lubrication", "status": "in_progress", "priority": "high",
     "description": "Low hydraulic pressure resolved via alert A295. Lubrication service in progress by hydraulic specialist."},
    {"id": "WO1038", "machine_id": "M118", "technician_id": "T01",
     "type": "inspection",  "status": "completed",   "priority": "low",
     "description": "Routine belt tension inspection — runtime approaching 500h threshold."},
]

FAULT_HISTORY = [
    {"id": "FH801", "machine_id": "M104", "fault_type": "High vibration 8.1 mm/s",
     "root_cause": "Front bearing wear",
     "resolution": "Bearing replacement (SKF 6205)",
     "downtime_hours": 2.1},
    {"id": "FH802", "machine_id": "M104", "fault_type": "Spindle noise",
     "root_cause": "Loose collet — not bearing",
     "resolution": "Collet re-torque",
     "downtime_hours": 0.5},
    {"id": "FH810", "machine_id": "M312", "fault_type": "Overtemperature 91°C",
     "root_cause": "Oil seal failure — not refrigerant",
     "resolution": "Seal replacement + oil flush",
     "downtime_hours": 4.0},
    {"id": "FH795", "machine_id": "M207", "fault_type": "Low hydraulic pressure",
...
TECHNICIANS = [
    {"id": "T01", "name": "Marcus Webb",  "specialty": "mechanical", "shift": "morning",   "availability": "available",
     "certifications": "Millwright Level 3, Conveyor Systems"},
    {"id": "T03", "name": "Diana Cruz",   "specialty": "mechanical", "shift": "morning",   "availability": "available",
     "certifications": "CNC Machining, Bearing & Drive Systems"},
    {"id": "T07", "name": "Rajan Patel",  "specialty": "hydraulic",  "shift": "morning",   "availability": "on_job",
     "certifications": "Hydraulic Systems Level 4, Fluid Power"},
    {"id": "T11", "name": "Sofia Lin",    "specialty": "electrical", "shift": "afternoon", "availability": "off_shift",
     "certifications": "PLC Programming, Motor Controls"},
    {"id": "T14", "name": "Owen Brandt",  "specialty": "pneumatic",  "shift": "morning",   "availability": "available",
     "certifications": "Pneumatic Systems, Valve Maintenance"},
]

PARTS = [
    {"id": "P201", "name": "SKF 6205 Bearing",     "category": "bearing", "compatible_machine_type": "cnc_mill",
     "stock_level": 4, "reorder_point": 2, "lead_time_days": 3},
    {"id": "P202", "name": "SKF 6305 Bearing",     "category": "bearing", "compatible_machine_type": "cnc_mill",
     "stock_level": 1, "reorder_point": 2, "lead_time_days": 3},
    {"id": "P310", "name": "Compressor Oil Seal",  "category": "seal",    "compatible_machine_type": "compressor",
     "stock_level": 2, "reorder_point": 1, "lead_time_days": 5},
    {"id": "P411", "name": "Hydraulic Valve Seal", "category": "seal",    "compatible_machine_type": "hydraulic_press",
     "stock_level": 3, "reorder_point": 2, "lead_time_days": 4},
    {"id": "P508", "name": "Conveyor Drive Belt",  "category": "belt",    "compatible_machine_type": "conveyor",
...
# owner_id used for all pre-seeded memories — the agent reads memory scoped to
# its own owner_id (set in cli.py). Use a shared floor-level id so all users
# can recall these facts via search_memory.
...
async def seed_memories() -> None:
    ...
    # Stamp each memory with a unique id based on current time to allow reruns.
    # Using fixed ids means reruns are idempotent (service deduplicates by id).
