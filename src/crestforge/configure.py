...
The agent key is written to ``_agentkey.tmp`` at the project root (git-ignored).

Run:
    python -m src.crestforge.configure           # create/update + mint agent key
...
# Resolve relative to the project root (3 levels up from src/crestforge/configure.py).
_PROJECT_ROOT = Path(__file__).parents[2]
KEY_OUT = str(_PROJECT_ROOT / "_agentkey.tmp")


# ---------------------------------------------------------------------------
# Entity models
# Field index type controls which MCP tool is generated:
...
class Machine(ContextModel):
    ...
    type: str = ContextField(
        description="cnc_mill / hydraulic_press / conveyor / compressor / robot_arm",
        index="tag",
    )
    location: str = ContextField(
        description="Zone: zone_a / zone_b / zone_c / zone_d", index="tag"
    )
    status: str = ContextField(
        description="running / idle / fault / maintenance", index="tag"
    )
    vibration_level: float = ContextField(
        description="Vibration in mm/s", index="numeric"
    )
    temperature_c: float = ContextField(
        description="Temperature in Celsius", index="numeric"
    )
    runtime_hours: float = ContextField(
        description="Hours since last major service", index="numeric"
    )
...
class Alert(ContextModel):
    ...
    machine_id: str = ContextField(
        description="Machine that triggered the alert", index="tag"
    )
    type: str = ContextField(
        description="vibration / temperature / pressure / lubrication / current_draw",
        index="tag",
    )
    severity: str = ContextField(
        description="info / warning / critical", index="tag"
    )
    status: str = ContextField(
        description="open / acknowledged / resolved", index="tag"
    )
    triggered_value: float = ContextField(
        description="Sensor reading that triggered the alert", index="numeric"
    )
    threshold: float = ContextField(
        description="Threshold value that was crossed", index="numeric"
    )
...
class WorkOrder(ContextModel):
    ...
    id: str = ContextField(
        description="Work order id (e.g. WO1041)", is_key_component=True
    )
    machine_id: str = ContextField(
        description="Machine the work order is for", index="tag"
    )
    technician_id: str = ContextField(
        description="Assigned technician id", index="tag"
    )
    type: str = ContextField(
        description="inspection / repair / replacement / lubrication", index="tag"
    )
    status: str = ContextField(
        description="scheduled / in_progress / completed / cancelled", index="tag"
    )
    priority: str = ContextField(
        description="low / medium / high / urgent", index="tag"
    )
    description: str = ContextField(
        description="Work order description and notes", index="text"
    )
...
class FaultHistory(ContextModel):
    ...
    id: str = ContextField(
        description="Fault history id (e.g. FH801)", is_key_component=True
    )
    machine_id: str = ContextField(
        description="Machine the fault occurred on", index="tag"
    )
    fault_type: str = ContextField(
        description="Description of the fault symptom", index="text"
    )
    root_cause: str = ContextField(
        description="Confirmed root cause of the fault", index="text"
    )
    resolution: str = ContextField(
        description="How the fault was resolved", index="text"
    )
    downtime_hours: float = ContextField(
        description="Total downtime in hours", index="numeric"
    )
...
class Technician(ContextModel):
    ...
    id: str = ContextField(
        description="Technician id (e.g. T03)", is_key_component=True
    )
    name: str = ContextField(description="Technician full name", index="text")
    specialty: str = ContextField(
        description="mechanical / electrical / hydraulic / pneumatic", index="tag"
    )
    shift: str = ContextField(
        description="morning / afternoon / night", index="tag"
    )
    availability: str = ContextField(
        description="available / on_job / off_shift", index="tag"
    )
    certifications: str = ContextField(
        description="Certifications and qualified systems", index="text"
    )
...
class Part(ContextModel):
    ...
    category: str = ContextField(
        description="bearing / belt / seal / sensor / motor / filter", index="tag"
    )
    compatible_machine_type: str = ContextField(
        description="Machine type this part fits", index="tag"
    )
    stock_level: int = ContextField(
        description="Units currently in stock", index="numeric"
    )
    reorder_point: int = ContextField(
        description="Stock level that triggers a reorder", index="numeric"
    )
    lead_time_days: int = ContextField(
        description="Supplier lead time in days", index="numeric"
    )
...
async def main() -> int:
    ...
    async with ContextSurfacesClient() as client:
        ...
        for s in surfaces:
            print(
                f"  - {getattr(s, 'name', '?')}  id={getattr(s, 'id', '?')}  "
                f"status={getattr(s, 'status', '?')}  "
                f"tools={len(getattr(s, 'tools', []) or [])}"
            )
        if probe:
            return 0
        ...
        match = next(
            (s for s in surfaces if getattr(s, "name", None) == SURFACE_NAME), None
        )
        if match:
            surface = await client.update_context_surface(
                match.id,
                UpdateContextSurfaceRequest(
                    data_model=data_model, data_source=data_source
                ),
                admin_key=admin_key,
            )
        ...
        print(
            "final status:", getattr(surface, "status", "?"),
            "| tools:", len(getattr(surface, "tools", []) or []),
        )

        agent_key = await client.create_agent_key(
            surface.id,
            CreateAgentKeyRequest(name="crestforge-agent"),
            admin_key=admin_key,
        )
        ...
        with open(KEY_OUT, "w", encoding="ascii") as f:
            f.write(key_val)
        print(
            f"agent key minted: {key_val[:6]}...(len {len(key_val)}) "
            f"-> wrote {os.path.basename(KEY_OUT)}"
        )
